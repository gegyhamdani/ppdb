<?php
	...
	$pdf->ln(1);
	$pdf->Cell(26.5,0.5,"LAPORAN PEKERJAAN ORANG TUA (IBU)",0,10,'C');
	$pdf->ln(1);
	$pdf->SetFont('Arial','B',10);
	...
	$pdf->Cell(4.5,0.5,"Catatan : Laporan Total per Pekerjaan Orang Tua (Ibu) dari Pendaftar Calon Siswa .",0,10,'L');
	$pdf->ln();
	$pdf->ln();
	$pdf->SetFont('Arial','B',10);
	...
	$sql2 = "SELECT count(pkrjn_ibu) as jumlah, pkrjn_ibu FROM biodata group by pkrjn_ibu";
	$result2 = $mydb->query($sql2);
	$no=1;
	while ($lihat = $result2->fetch_assoc()):
		$pdf->Cell(1, 0.8, $no , 1, 0, 'C');
		$pdf->Cell(10, 0.8, $lihat['pkrjn_ibu'],1, 0, 'C');
		$pdf->Cell(3, 0.8, $lihat['jumlah'], 1, 0,'C');
		$pdf->ln();
		$no++;
	endwhile;
	$pdf->Output("Laporan Pekerjaan Orang Tua Ibu.pdf","I");
?>
