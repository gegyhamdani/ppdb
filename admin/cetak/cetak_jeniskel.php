<?php
	...
	$pdf->ln(1);
	$pdf->Cell(26.5,0.5,"LAPORAN JENIS KELAMIN",0,10,'C');
	$pdf->ln(1);
	$pdf->SetFont('Arial','B',10);
	...
	$pdf->Cell(4.5,0.5,"Catatan : Laporan Total per Jenis Kelamin dari Pendaftar Calon Siswa .",0,10,'L');
	$pdf->ln();
	$pdf->ln();
	$pdf->SetFont('Arial','B',10);
	...
	$sql2 = "SELECT count(jeniskel) as jumlah, jeniskel FROM biodata group by jeniskel";
	$result2 = $mydb->query($sql2);
	$no=1;
	while ($lihat = $result2->fetch_assoc()):
		$pdf->Cell(1, 0.8, $no , 1, 0, 'C');
		$pdf->Cell(10, 0.8, $lihat['jeniskel'],1, 0, 'C');
		$pdf->Cell(3, 0.8, $lihat['jumlah'], 1, 0,'C');
		$pdf->ln();
		$no++;
	endwhile;
	$pdf->Output("Laporan Jenis Kelamin.pdf","I");
?>
