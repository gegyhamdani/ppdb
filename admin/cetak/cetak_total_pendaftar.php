<?php
	...
	$pdf->ln(1);
	$pdf->Cell(26.5,0.5,"LAPORAN TOTAL PENDAFTAR",0,10,'C');
	$pdf->ln(1);
	$pdf->SetFont('Arial','B',10);
	...
	$pdf->Cell(4.5,0.5,"Catatan : Laporan Pendaftaran Calon Siswa .",0,10,'L');
	$pdf->ln();
	$pdf->ln();
	$pdf->SetFont('Arial','B',10);
	$pdf->Cell(4, 0.8, 'Total Pendaftar', 1, 0, 'C');
	$pdf->ln();	
	$pdf->SetFont('Arial','',10);
	
	
	$sql = "SELECT count(*) as jumlah FROM biodata";
	...
	$pdf->Output("Laporan Total Pendaftar.pdf","I");
?>
